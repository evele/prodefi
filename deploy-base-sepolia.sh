...
FIREBASE_FUNCTIONS_BASE_URL=$(get_existing_value "$FRONTEND_ENV_FILE" "VITE_FIREBASE_FUNCTIONS_BASE_URL")
MERCADO_PAGO_USE_SANDBOX=$(get_existing_value "$FRONTEND_ENV_FILE" "VITE_MERCADO_PAGO_USE_SANDBOX")
...
if [ -z "$FIREBASE_FUNCTIONS_BASE_URL" ]; then
  FIREBASE_FUNCTIONS_BASE_URL="https://southamerica-east1-prodefi-f2237.cloudfunctions.net"
fi

if [ -z "$MERCADO_PAGO_USE_SANDBOX" ]; then
...
VITE_FIREBASE_FUNCTIONS_BASE_URL=$FIREBASE_FUNCTIONS_BASE_URL
VITE_MERCADO_PAGO_USE_SANDBOX=$MERCADO_PAGO_USE_SANDBOX
...
echo ""
echo "Next steps:"
echo "  1. Check Openfort keys and optional sponsorship values in $FRONTEND_ENV_FILE"
